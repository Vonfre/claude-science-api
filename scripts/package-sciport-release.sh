...
)
{
  ...
  printf 'Built by GitHub Actions; no automated test suites or live-provider tests run.\n'
...
- **按维护者要求，此次直接由 GitHub 构建发布，未运行自动化测试套件，实际使用由维护者验证。构建成功不代表功能全部通过。**
...
- **At the maintainer's request, this release is built on GitHub without automated test suites. Hands-on validation is left to the maintainer. A successful build is not proof that all functionality works.**
